...
1. **信息准确**
   ...
   - **严禁翻译和生硬机翻专业术语**：
     ...
     - **芯片后端名词绝大部分不翻译，直接使用英文**：
       - 诸如 `net`, `place`, `placement`, `routing`, `cell`, `pin`, `port`, `macro`, `die`, `core` 等基础实体和动作词汇，**必须全部保留英文原文**，绝对不要翻译成中文（例如：绝对不能把 net 翻译成网络或线网，把 routing 翻译成路由或布线，把 placement 翻译成放置或布局等）。
       - 如遇到任何不确定的专业词汇，**一律保留英文原文，绝不能进行机翻或日常用语直译**。
